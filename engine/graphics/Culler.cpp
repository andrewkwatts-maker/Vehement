...
AABB AABB::FromMesh(const Mesh& mesh) {
    return AABB(mesh.GetBoundsMin(), mesh.GetBoundsMax());
}
...
int Frustum::TestAABB(const AABB& aabb) const {
...
int Frustum::TestSphere(const glm::vec3& center, float radius) const {
    int result = 1;
